# Cartesian product of hamming graph
#-------How to run---
# Input Data- 
#1. Enter the value of n1 and d1 for graph G(n1,d1)
#2. Enter the value of n2 and d2 for graph G(n2,d2)

#It will print the number of vertices of the cartesian product followed by vertices.
#Then, the edges between the vertices of the cartesian product followed by total number of edges.
...
num_of_vertices_in_cp= 2**(n1+n2)

for i in range(0,num_of_vertices_g1):
    for j in range(0,num_of_vertices_g2):
        cartesian_graph_vertex=str(v1[i])+str(v2[j])
        v3.append(cartesian_graph_vertex)

#print the vertex set of the cartesian product
print("The total number of vertices in cartesian product:= ")
print (num_of_vertices_in_cp)
print("The vertices of the cartesian product are:")
print(v3)

print ("The edges exist between the following vertices:")
edge_ok=1
num_of_edges_in_cp=0
#checking for edges in the 2nd graph, keeping 1st as constant
for i in range(0,num_of_vertices_in_cp):
    for j in range(i+1,num_of_vertices_in_cp):
        edge_ok=1
        l1=v3[i][:n1]
        l2=v3[j][:n1]
        r1=v3[i][n1:]
        ...
        if(str(l1)==str(l2)):
            for k in range(0,len(e2)):
                if((str(e2[k][0])==str(r1) and str(e2[k][1])==str(r2))):
                    cp_l=l1+r1
                    cp_r=l2+r2
                    num_of_edges_in_cp=num_of_edges_in_cp+1
                    print (cp_l,cp_r)
             
#checking for edges in the 1st graph, keeping 2nd as constant
for i in range(0,num_of_vertices_in_cp):
    for j in range(i+1,num_of_vertices_in_cp):
        edge_ok=1
        l1=v3[i][:n1]
        ...
        if(str(r1)==str(r2)):
            for k in range(0,len(e1)):
                if( (str(e1[k][0])==str(l1) and str(e1[k][1])==str(l2))):
                    cp_l=l1+r1
                    cp_r=l2+r2
                    num_of_edges_in_cp=num_of_edges_in_cp+1
                    print (cp_l,cp_r)
print("The total number of edges in cartesian product:= ")
print (num_of_edges_in_cp)
